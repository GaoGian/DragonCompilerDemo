...
/**
 * 解析正则表达式，拆分成元字符，摘抄 LexPattern 代码
...
public class LexPattern {
    ...
    public static LexPattern compile(String regex) {
        return new LexPattern(regex, 0);
    }

    public static LexPattern compile(String regex, int flags) {
        return new LexPattern(regex, flags);
    }
    ...
    public LexMatcher matcher(CharSequence input) {
        if (!compiled) {
            synchronized (this) {
                if (!compiled) {
                    compile();
        ...
        LexMatcher m = new LexMatcher(this, input);
        return m;
    }
    ...
    public static boolean matches(String regex, CharSequence input) {
        LexPattern p = LexPattern.compile(regex);
        LexMatcher m = p.matcher(input);
        return m.matches();
    }
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        // if length > 0, the LexPattern is lazily compiled
        compiled = false;
        if (pattern.length() == 0) {
    ...
    private LexPattern(String p, int f) {
        pattern = p;
        flags = f;
    ...
    private void RemoveQEQuoting() {
        ...
        while (i < pLen) {
            int c = temp[i++];
            if (!LexASCII.isAscii(c) || LexASCII.isAlpha(c)) {
                newtemp[j++] = c;
            } else if (LexASCII.isDigit(c)) {
                if (beginQuote) {
                    /*
    ...
    private int peekPastWhitespace(int ch) {
        while (LexASCII.isSpace(ch) || ch == '#') {
            while (LexASCII.isSpace(ch))
                ch = temp[++cursor];
            if (ch == '#') {
                ch = peekPastLine();
            }
        ...
        switch (ch) {
            ...
            case 'D':
                if (create) root = has(UNICODE_CHARACTER_CLASS)
                        ? new Utype(LexUnicodeProp.DIGIT).complement()
                        : new Ctype(LexASCII.DIGIT).complement();
                return -1;
            case 'E':
            case 'F':
                break;
            ...
            case 'S':
                if (create) root = has(UNICODE_CHARACTER_CLASS)
                        ? new Utype(LexUnicodeProp.WHITE_SPACE).complement()
                        : new Ctype(LexASCII.SPACE).complement();
                return -1;
            case 'T':
            case 'U':
                break;
            ...
            case 'W':
                if (create) root = has(UNICODE_CHARACTER_CLASS)
                        ? new Utype(LexUnicodeProp.WORD).complement()
                        : new Ctype(LexASCII.WORD).complement();
                return -1;
            case 'X':
            case 'Y':
                break;
            ...
            case 'd':
                if (create) root = has(UNICODE_CHARACTER_CLASS)
                        ? new Utype(LexUnicodeProp.DIGIT)
                        : new Ctype(LexASCII.DIGIT);
                return -1;
            case 'e':
                return '\033';
            case 'f':
            ...
            case 's':
                if (create) root = has(UNICODE_CHARACTER_CLASS)
                        ? new Utype(LexUnicodeProp.WHITE_SPACE)
                        : new Ctype(LexASCII.SPACE);
                return -1;
            case 't':
                return '\t';
            case 'u':
            ...
            case 'w':
                if (create) root = has(UNICODE_CHARACTER_CLASS)
                        ? new Utype(LexUnicodeProp.WORD)
                        : new Ctype(LexASCII.WORD);
                return -1;
            case 'x':
                return x();
            case 'y':
    ...
    private CharProperty family(boolean singleLetter,
        ...
        if (i != -1) {
            ...
            if (name.startsWith("In")) {
                ...
                LexUnicodeProp uprop = LexUnicodeProp.forName(name);
                if (uprop != null) {
                ...
                if (has(UNICODE_CHARACTER_CLASS)) {
                    LexUnicodeProp uprop = LexUnicodeProp.forPOSIXName(name);
                    if (uprop != null) {
                        node = new Utype(uprop);
                    }
                }
    ...
    private String groupname(int ch) {
        ...
        while (LexASCII.isLower(ch = read()) || LexASCII.isUpper(ch) ||
                LexASCII.isDigit(ch)) {
            sb.append(Character.toChars(ch));
        }
        if (sb.length() == 0)
        ...
        if (ch == '?') {
            ...
            switch (ch) {
                ...
                case '<':   // (?<xxx)  look behind
                    ch = read();
                    if (LexASCII.isLower(ch) || LexASCII.isUpper(ch)) {
    ...
    private Node closure(Node prev) {
        ...
        switch (ch) {
            ...
            case '{':
                ch = temp[cursor + 1];
                if (LexASCII.isDigit(ch)) {
                    skip();
                    int cmin = 0;
                    do {
                        cmin = cmin * 10 + (ch - '0');
                    } while (LexASCII.isDigit(ch = read()));
                    int cmax = cmin;
                    if (ch == ',') {
                        ch = read();
                        cmax = MAX_REPS;
    ...
    private int x() {
        int n = read();
        if (LexASCII.isHexDigit(n)) {
            int m = read();
            if (LexASCII.isHexDigit(m)) {
                return LexASCII.toDigit(n) * 16 + LexASCII.toDigit(m);
            }
        } else if (n == '{' && LexASCII.isHexDigit(peek())) {
            int ch = 0;
            while (LexASCII.isHexDigit(n = read())) {
                ch = (ch << 4) + LexASCII.toDigit(n);
    ...
    private int uxxxx() {
        ...
        for (int i = 0; i < 4; i++) {
            int ch = read();
            if (!LexASCII.isHexDigit(ch)) {
                throw error("Illegal Unicode escape sequence");
            }
            n = n * 16 + LexASCII.toDigit(ch);
        }
        return n;
    }
    ...
    private static final class BitClass extends BmpCharProperty {
        ...
        BitClass add(int c, int flags) {
            ...
            if ((flags & CASE_INSENSITIVE) != 0) {
                if (LexASCII.isAscii(c)) {
                    bits[LexASCII.toUpper(c)] = true;
                    bits[LexASCII.toLower(c)] = true;
    ...
    private CharProperty newSingle(final int ch) {
        if (has(CASE_INSENSITIVE)) {
            ...
            } else if (LexASCII.isAscii(ch)) {
                lower = LexASCII.toLower(ch);
                upper = LexASCII.toUpper(ch);
                if (lower != upper)
    ...
    static class LastNode extends Node {
        ...
        boolean match(LexMatcher matcher, int i, CharSequence seq) {
            if (matcher.acceptMode == LexMatcher.ENDANCHOR && i != matcher.to)
                return false;
            matcher.last = i;
            matcher.groups[0] = matcher.first;
            matcher.groups[1] = matcher.last;
    ...
    static final class Utype extends CharProperty {
        final LexUnicodeProp uprop;

        Utype(LexUnicodeProp uprop) {
            this.uprop = uprop;
        }
    ...
    static final class Ctype extends BmpCharProperty {
        ...
        boolean isSatisfiedBy(int ch) {
            return ch < 128 && LexASCII.isType(ch, ctype);
        }
    }
    ...
    private CharProperty caseInsensitiveRangeFor(final int lower,
        ...
        return new CharProperty() {
            boolean isSatisfiedBy(int ch) {
                return inRange(lower, ch, upper) ||
                        LexASCII.isAscii(ch) &&
                                (inRange(lower, LexASCII.toUpper(ch), upper) ||
                                        inRange(lower, LexASCII.toLower(ch), upper));
            }
        };
    }
    ...
    static final class Curly extends Node {
        ...
        boolean match0(LexMatcher matcher, int i, int j, CharSequence seq) {
            if (j >= cmax) {
        ...
        // i is the index to start matching at
        // j is the number of atoms that have matched
        boolean match1(LexMatcher matcher, int i, int j, CharSequence seq) {
            for (; ; ) {
        ...
        boolean match2(LexMatcher matcher, int i, int j, CharSequence seq) {
            for (; j < cmax; j++) {
                if (!atom.match(matcher, i, seq))
                    break;
                if (i == matcher.last)
    ...
    static final class GroupHead extends Node {
        ...
        boolean matchRef(LexMatcher matcher, int i, CharSequence seq) {
            int save = matcher.locals[localIndex];
    ...
    static class Loop extends Node {
        ...
        boolean matchInit(LexMatcher matcher, int i, CharSequence seq) {
            ...
            for (int index = 0; index < groupSize; index++) {
                ...
                if (c1 != c2) {
                    if (doUnicodeCase) {
                        ...
                        if (LexASCII.toLower(c1) != LexASCII.toLower(c2))
                            return false;
                    }
                }
    ...
    static final class Bound extends Node {
        ...
        boolean isWord(int ch) {
            return useUWORD ? LexUnicodeProp.WORD.is(ch)
                    : (ch == '_' || Character.isLetterOrDigit(ch));
        }

        int check(LexMatcher matcher, int i, CharSequence seq) {
            int ch;
            boolean left = false;
            int startIndex = matcher.from;
            int endIndex = matcher.to;
    ...
    private static boolean hasBaseCharacter(LexMatcher matcher, int i,
                                            CharSequence seq) {
...
///////////////////////////////////////////////////////////////////////////////
    ...
    private static class CharPropertyNames {
        ...
        static {
            ...
            defRange("LexASCII", 0x00, 0x7F);   // LexASCII
            defCtype("Alnum", LexASCII.ALNUM);  // Alphanumeric characters
            defCtype("Alpha", LexASCII.ALPHA);  // Alphabetic characters
            defCtype("Blank", LexASCII.BLANK);  // Space and tab characters
            defCtype("Cntrl", LexASCII.CNTRL);  // Control characters
            defRange("Digit", '0', '9');     // Numeric characters
            defCtype("Graph", LexASCII.GRAPH);  // printable and visible
            defRange("Lower", 'a', 'z');     // Lower-case alphabetic
            defRange("Print", 0x20, 0x7E);   // Printable characters
            defCtype("Punct", LexASCII.PUNCT);  // Punctuation characters
            defCtype("Space", LexASCII.SPACE);  // Space characters
            defRange("Upper", 'A', 'Z');     // Upper-case alphabetic
            defCtype("XDigit", LexASCII.XDIGIT); // hexadecimal digits
