...
//            lookSymbolSet.addAll(item.getLookForwardSymbolSet());
     ...
     * FIXME 2019-07-04
     * LR1推导过程推理：
     * 1、首先LR文法分析与LL文法分析不同，在读入文法符号时不能唯一确认产生式，只能推导出当前文法可能处于的状态，这点类似于DFA，LR项集对应DFA状态
     * 2、LR文法在推导过程中需要判断何时可以进行规约操做，以便唯一确认产生式，假设当前输入栈已经读入多个文法符号，并且处于栈顶的前几个符号可以构成符合当前状态项集中的某个产生式的句型，
     *    这里需要考虑几种情况：
     *      A、同时符合多个产生式（这个放到动作冲突上讲）；
     *      B、是直接根据现在的符号进行规约还是继续读入，和后续输入形成句型，这里有几种情况
     *          ①、规约后是否可以继续规约（需要考虑规约后的符号是和后续输入构成句型，还是和输入栈中的符号形成句柄继续规约，后者可能正确的处理是应该是和后续符号拼接，此时会造成过度规约（不是可行句柄））；
     *              例如一个橘子“句子 → 我·早上起来·坐地铁·去公司上班”，在这个句子的输入过程中有多个位置可以认为已存在完整的句子（例如“我”也是一个完整的句子），但是如果直接规约的话会造成识别不全的问题，
     *              那么后续输入就无法继续识别（从文法结构上当前规约的符号无法和后续输入组成句型，造成识别失败），因此应该优先读取尽可能多的信息作为识别的依据，保证句子阶段性完整识别（例如遇到终结符或者非该文法符号定义的输入），
     *              关键是如何判断当前文法符号所对应的是否已经读完了;
     *          ②、文法结合性问题，当前的符号需要优先和后续符号组成句型，例如：a + b * c
     *              这个问题可以通过设定文法的优先性（文法的最小句型）解决，例如：exp → exp + term、term → factor * factor，只有识别出term后才能组成exp句型，利用自底向上的原则，从局部到整体保证句型的优先结合；
     *          ③、二义性问题，例如：if stmt else if stmt else stmt "then"
     *              这里的问题是最后的“then”既可以认为是内部的if else的一部分，也可以认为是外部if else的一部分，
     *              这种情况需要设定其他规则进行识别（例如就近匹配原则），文法上通过分离两种句型实现，
     *                  例如：IF -> if stmt else stmt、stmt -> if IF else IF then，这样的话词法单元“then”总汇优先和最近的if else组成if else then句型
     *              （为什么不使用就远原则，因为LR分析推导是不进行计数的，因此不清楚应该要找多远前的符号进行匹配，需要额外记录其他信息，这就需要针对特定需求有不同的推导，不符合通用LR分析，可以考虑放入到语义分析阶段自定义解释）
     * 3、为了解决问题 B.①，需要在做规约推导的时候引入之前推导的上文情况，既之前的推导链，记录目前的的产生式是从哪个文法的哪个位置推导而来，只有当输入符号是目标规约符号的后续符号时才能说明当前是可归约状态；
     *      S→α·Aβ，A→xyz·，next_read=b，如果 b∈first(β)，那么xyz==>A成立
     *    这里需要说明几个问题
     *      A、这里的 b 是来自 A 的向前看符号既FIRST，当LR进行推导时，可以从从上层文法带入到下层文法符号（目标推导文法符号的后一文法符号FIRST集）；
     *      B、为什么使用first集合，而不考虑follow，个人认为是因为follow在符号处于产生式末尾时需要考虑到产生式头部的follow集合，因此需要上层符号是已确认的情况（自底向上文法），因此是不可行的（是否可以推迟到输入完毕确认？？），
     *         而first集合只需要目标文法符号的后一符号就行，这个可以在推导的过程中一层层往下带，是可确认的；
     *      C、该规约推导需要满足一定的条件，既规约项与其他项的first集合不能有重合的部分，否则会造成 移入/规约、规约/规约冲突（问题2.A），而造成first重合的原因是因为定义的文法存在句型相同的情况（不一定是单个产生式，也可能是下层产生式和上层产生式的后续符号组成的句型），
     *         例如：产生式 1（S→α·A，[b/c]∈first(B)(上层后继符号统一标识)）、产生式2（L→α·Aγ，b∈first(γ)）、next_read=b，这两个产生式存在 移入/规约冲突，原因就是就读入符号b后，产生式1和产生式2都可与符号b形成αAb 句型，
     *               说明两个产生式形成的句型有重叠的部分，进而说明使用的文法存在相似定义的情况；
     *         解决方法：
     *              ①、增加非终结符，进一步区分不同的文法结构；
     *              ②、将文法结构相似的文法整合成一个；
     *                  （这里造成的问题可能是之前需要通过两个文法表示不同的语义，现在整合成一个后就造成无法分开处理。
     *                      解决方法是：推迟语义表达的实际，等到识别出当前产生式属于不同上层文法的时候再进行表达）
     *
     */
